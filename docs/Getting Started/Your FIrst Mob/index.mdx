...
It is a ranged mob that shoots fireballs at the target.
...
<FileTree
    paths={[
        ...
        "Mobs/highlight:DemonicMage.yml",
        "...",
    ]}
    title={<h3>MythicMobs folder</h3>}
/>
...
Since MythicMobs is a plugin, it cannot actually create custom entity types, so we have to base our mob off an existing entity type.
...
The `&4` and `&7` are colorcodes, which tell MythicMobs to color the text red and grey respectively.
The `☠` is a unicode character, which renders as a skull.
...
Finally, lets make the mob immune to all knockback. We can do this by setting its knockback resistance to 1 (100%):
